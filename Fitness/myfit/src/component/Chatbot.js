...
export default function Chatbot() {
  ...
  return (
    <div style={{marginTop:"3cm"}}>
      ...
      <div style={{display:"flex",flexDirection:'column',alignItems:"center",justifyContent:"center",marginTop:"1cm"}}>
        <input type="text" value={query} onChange={handleInputChange} placeholder="Ask your doubts" style={{width:"50%",color:"black"}}/>
        <Button onClick={handleSubmit} style={{backgroundColor:'purple',color:"white",marginTop:'0.5cm'}}>Submit</Button>
        {response && <Typography sx={{textAlign:'center',marginTop:'0.8cm'}}>{response}</Typography>}
      </div>
    </div>
  );
}
